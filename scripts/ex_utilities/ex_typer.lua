...
--	Make sure the GUI has a 'scriptDelta', and have the 'update' function in the .lua file call the 'text_typer.update' update
--	You !MUST! create a table (even an empty one) and pass it down to any function thats used in the typer
--	Should a function (button clicked, init, etc...) display text, just call 'text_typer.init' with the right parameters

--		Functions:
--	text_typer.init( textData, string, string )
...
--	text_typer.update( textData, widget, sound, volume, cutoffSound )
--		textData	= Table that holds data required for the script to function
...
text_typer = {}
text_typer.allowedScrambleCharacters = "abdeghnopqsuvyzABDGHJKNOPQRSUVXY023456789_~"

function text_typer.init(textData, str, sound)
	if not textData then
		sb.logError("text_typer.init in text_typer recieved no textData table, writing aborted.")
		return
	end
	...
	if textData.soundPlaying then
		text_typer.stopSounds(textData)
	...
	if textCopy == nil then
		textCopy = "^red;ERROR -^reset;\ntext_typer.init recieved a nil value in 'str'"
	elseif type(textCopy) ~= "string" then
		textCopy = "^red;ERROR -^reset;\ntext_typer.init recieved a non-string value in 'str'"
	end
			...
			elseif str == "[" then
				...
				if bracketEnd then
					...
					elseif string.find(format, "(data)") then
						...
						local location = text_typer.splitTableString(str)
						local data = textData
...
function text_typer.update(textData, wd, sound, volume, cutoffSound)
...
function text_typer.scrambling(textData)
...
function text_typer.stopSounds(textData)
