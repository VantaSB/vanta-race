...
function init()
	...
	message.setHandler("player.species", function (_, _, ...) return player.species() end)
	...
	message.setHandler("ex.giveItem", function(_,_,item) world.spawnItem(item, entity.position()) end)
	message.setHandler("ex.openLogbook", function (_, _, configData) player.interact("ScriptPane", configData) end)
