...
function populateQueslineList()
	...
	local questlineStatuses = status.statusProperty("vantaQuestlinesComplete", "")
	...
	for _, tbl in ipairs(QD.lines) do
		...
		if available then
			...
			widget.setImage(path..".banner", "/interface/scripted/corvexquestwindow/banners/"..tbl.id..".png:default")
		...
		elseif not tbl.secret then
			...
			widget.setImage(path..".banner", "/interface/scripted/corvexquestwindow/banners/"..tbl.id..".png:unavailable")
		end
	end
...
function questlineSelected()
	...
	if listItem then
		...
		local buttons = {
			base = "/interface/scripted/corvexquestwindow/questlineListOverlay.png:selected",
			hover = "/interface/scripted/corvexquestwindow/questlineListOverlay.png:selectedHover",
		}
		widget.setButtonImages(listItem..".overlay", buttons)

		if self.lastSelected and self.lastSelected ~= listItem then
			local lastButtons = {
				base = "/interface/scripted/corvexquestwindow/questlineListOverlay.png:default",
				hover = "/interface/scripted/corvexquestwindow/questlineListOverlay.png:hover",
			}
	...
	for questID, path in pairs(self.questListIDs) do
		...
		if config then
			if player.hasCompletedQuest(questID) then
				...
				widget.setImage(path..".status", "/interface/scripted/corvexquestwindow/status.png:complete")

			elseif player.hasQuest(questID) then
				widget.setText(path..".questName", tostring(config.title))
				widget.setFontColor(path..".questName", "cyan")
				widget.setImage(path..".status", "/interface/scripted/corvexquestwindow/status.png:inProgress")

			elseif player.canStartQuest(questID) then
				widget.setText(path..".questName", tostring(config.title))
				widget.setFontColor(path..".questName", "yellow")
				widget.setImage(path..".status", "/interface/scripted/corvexquestwindow/status.png:available")

			else
				if config.secret then
					widget.setText(path..".questName", "???")
					widget.setFontColor(path..".questName", "red")
					widget.setImage(path..".status", "/interface/scripted/corvexquestwindow/status.png:secret")
				else
					widget.setText(path..".questName", tostring(config.title))
					widget.setFontColor(path..".questName", "red")
					widget.setImage(path..".status", "/interface/scripted/corvexquestwindow/status.png:unavailable")
				end
			end
		else
				...
				for sublineNum, subline in ipairs(lineTbl.sublines) do
					for questNum, questID in ipairs(QD.sublines[subline]) do
						...
						if questNum == #QD.sublines[subline] then
							...
							widget.setImage(portrait, "/interface/scripted/corvexquestwindow/givers/"..subline..".png")
...
function questSelected()
	...
	if listItem then
		...
		if id == GD.portraitSpacingString then
			...
			widget.setImage(listItem..".bg", "/interface/scripted/corvexquestwindow/questBackground.png:selected")
			...
			if self.lastSelected and self.lastSelected ~= listItem then
				widget.setImage(self.lastSelected..".bg", "/interface/scripted/corvexquestwindow/questBackground.png:default")
			end
...
function questButton()
	if widget.active("questlineList") then
		...
		if path then
			...
			widget.setImage("questlineBanner", "/interface/scripted/corvexquestwindow/banners/"..questline..".png:default")
