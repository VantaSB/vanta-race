...
function init()
	...
	data = root.assetJson("/interface/scripted/ex_research/data.config")
...
function searchButton()
	...
	if widget.active("searchList") then
		...
		{	base = "/interface/scripted/ex_research/buttons.png:search:default",
			hover = "/interface/scripted/ex_research/buttons.png:search:hover"	})
	elseif researchTree then
		...
		widget.setButtonImages("searchButton",
		{	base = "/interface/scripted/ex_research/buttons.png:search:hover",
			hover = "/interface/scripted/ex_research/buttons.png:search:default"	})
	end
end
...
function treePickButton()
	...
	if widget.active("treeList") then
		...
		{	base = "/interface/scripted/ex_research/buttons.png:treePick:default",
			hover = "/interface/scripted/ex_research/buttons.png:treePick:hover"	})
	else
		...
		widget.setButtonImages("treePickButton",
		{	base = "/interface/scripted/ex_research/buttons.png:treePick:hover",
			hover = "/interface/scripted/ex_research/buttons.png:treePick:default"	})
	end
end
	...
	if not widget.active("searchList") and not widget.active("treeList") then
		...
		if selected then
			if data.strings.research[selected] then
				...
				-- New processor for masking quest names if they aren't in the player's journal
				local rawDescription = data.strings.research[selected][2]
				local processedDescription = processResearchDescription(rawDescription, researchTree[selected].requiredQuests or {})
				widget.setText("infoList.text", processedDescription)
				-- Old behavior
...
function processResearchDescription(raw, quests)
	if not string.find(raw, "<questname>") then
		return raw
	end

	if #quests == 0 then
		return raw:gsub("<questname>", "???")
	end

	for _, questId in ipairs(quests) do
		if player.hasQuest(questId) or player.hasCompletedQuest(questId) then
			local questTitle = root.questData(questId).title
			return raw:gsub("<questname>", questTitle)
		end
	end
	...
	for tree, _ in pairs(data.researchTree) do
		...
		if data.treeUnlocks[tree] then
			if data.treeUnlocks[tree].requiredQuests then
				for _, questId in ipairs(data.treeUnlocks[tree].requiredQuests) do
					sb.logInfo("Quest ID: ")
...
function draw()
	...
	if player.species() == "vanta" then
		...
		if not player.hasCompletedQuest("vantaintro") then
