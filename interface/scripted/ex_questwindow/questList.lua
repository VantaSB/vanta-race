...
function init()
	GD = root.assetJson("/interface/scripted/ex_questwindow/questList.config")
	QD = root.assetJson("/interface/scripted/ex_questwindow/data.config")

	for _, file in ipairs(QD.questlineFiles) do
		local temp = root.assetJson(file)
		QD = exutil.MergeTable(QD, temp)
	end
	...
	if not player.hasCompletedQuest("vantatutorial3") then
		widget.setVisible("questlineNoSelected", false)
		widget.setVisible("questlineButton", false)
		widget.setVisible("questButton", false)
		widget.setText("questTitle", "^#FF5E66;NOT AUTHORIZED^reset;")
		widget.setText("textScrollArea.questText", "Conditions have not yet been met in order to use the task terminal.")
	else
		questlineButton()
...
function populateQuestlineList()
	widget.clearListItems("questlineList.list")

	widget.setButtonEnabled("questButton", false)
	widget.setText("questButton", "Select")

	local questlineStatuses = status.statusProperty("exQuestlinesComplete", "")
	...
	for _, tbl in ipairs(QD.lines) do
		...
		if available then
			...
			widget.setImage(path..".banner", "/interface/scripted/ex_questwindow/banners/"..tbl.id..".png:default")
		...
		elseif not tbl.secret then
			...
			widget.setImage(path..".banner", "/interface/scripted/ex_questwindow/banners/"..tbl.id..".png:unavailable")
		end
	end
...
function questlineSelected()
	...
	if listItem then
		...
		local buttons = {
			base = "/interface/scripted/ex_questwindow/questlineListOverlay.png:selected",
			hover = "/interface/scripted/ex_questwindow/questlineListOverlay.png:selectedHover",
		}
		widget.setButtonImages(listItem..".overlay", buttons)

		if self.lastSelected and self.lastSelected ~= listItem then
			local lastButtons = {
				base = "/interface/scripted/ex_questwindow/questlineListOverlay.png:default",
				hover = "/interface/scripted/ex_questwindow/questlineListOverlay.png:hover",
			}
	...
	for questID, path in pairs(self.questListIDs) do
		...
		if config then
			if player.hasCompletedQuest(questID) then
				...
				widget.setImage(path..".status", "/interface/scripted/ex_questwindow/status.png:complete")

			elseif player.hasQuest(questID) then
				widget.setText(path..".questName", tostring(config.title))
				widget.setFontColor(path..".questName", "cyan")
				widget.setImage(path..".status", "/interface/scripted/ex_questwindow/status.png:inProgress")

			elseif player.canStartQuest(questID) then
				widget.setText(path..".questName", tostring(config.title))
				widget.setFontColor(path..".questName", "yellow")
				widget.setImage(path..".status", "/interface/scripted/ex_questwindow/status.png:available")

			else
				if config.secret then
					widget.setText(path..".questName", "???")
					widget.setFontColor(path..".questName", "red")
					widget.setImage(path..".status", "/interface/scripted/ex_questwindow/status.png:secret")
				else
					widget.setText(path..".questName", tostring(config.title))
					widget.setFontColor(path..".questName", "red")
					widget.setImage(path..".status", "/interface/scripted/ex_questwindow/status.png:unavailable")
				end
			end
		else
				...
				for sublineNum, subline in ipairs(lineTbl.sublines) do
					for questNum, questID in ipairs(QD.sublines[subline]) do
						...
						if questNum == #QD.sublines[subline] then
							...
							widget.setImage(portrait, "/interface/scripted/ex_questwindow/givers/"..subline..".png")
...
function questSelected()
	...
	if listItem then
		...
		if id == GD.portraitSpacingString then
			...
			widget.setImage(listItem..".bg", "/interface/scripted/ex_questwindow/questBackground.png:selected")
			...
			if self.lastSelected and self.lastSelected ~= listItem then
				widget.setImage(self.lastSelected..".bg", "/interface/scripted/ex_questwindow/questBackground.png:default")
			end
...
function questButton()
	if widget.active("questlineList") then
		...
		if path then
			...
			widget.setImage("questlineBanner", "/interface/scripted/ex_questwindow/banners/"..questline..".png:default")
