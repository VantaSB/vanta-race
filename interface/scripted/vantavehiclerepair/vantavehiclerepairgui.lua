...
function repairVehicle(item)
  ...
  if player.consumeItem(item) then
    local consumed = player.consumeItem({name = "autochip", count = cost}, true)
    local repairedItem = copy(item)
    if consumed then
      local repairAmount = consumed.count * self.chipRepairAmount
      repairedItem.parameters.vehicleStartHealthFactor = math.min(1.0, vehicleDurability(repairedItem) + repairAmount)
...
function showVehicle(item)
  local chips = player.hasCountOfItem("autochip")
  local enableButton = false

  if item and repairCost(item) > 0 then
