...
require "/scripts/ex_utilities/ex_typer.lua"
...
text_typer.initOrig = text_typer.init
function text_typer.init(textData, str, sound)
	doneTalking = false
	text_typer.initOrig(textData, str, sound)
end

text_typer.skipOrig = text_typer.skip
function text_typer.skip(textData, wd)
	doneTalking = false
	text_typer.skipOrig(textData, wd)
end
...
function enableFailsafe(err)
	...
	sb.logError("")
	sb.logError("ERROR WHILE EXECUTING CUSTOM SAIL:")
	sb.logError("%s", err)
	sb.logError("")
end
...
function initSafe()
	cfg = root.assetJson("/interface/scripted/ex_sail/data.config")
	...
	buildCurrencyList()
	text_typer.init(cfg.TextData, cfg.TextData.strings.intro, customData.chatterSound or cfg.TextData.sound)
end
...
function resetGUI()
	...
	text_typer.skip(cfg.TextData, "root.text")
...
function updateSafe(dt)
	...
	if cfg.TextData.isFinished then
		...
		if 1 / (customData.charactersPerSecond or GUI.talker.charactersPerSecond) <= charDeltaCounter then
			text_typer.scrambling(cfg.TextData)
			text_typer.update(cfg.TextData, "root.text")
			charDeltaCounter = 0
		end
	end
...
function draw()
	...
	GUI.canvas:drawImage(GUI.static.image..":"..GUI.static.frame, {0,0}, nil, "#FFFFFF"..exutil.ValToHex(customData.staticOpacity or GUI.static.opacity), false)
	GUI.canvas:drawImage(GUI.scanLines.image..":"..GUI.scanLines.frame, {0,0}, nil, "#FFFFFF"..exutil.ValToHex(customData.scanlineOpacity or GUI.scanLines.opacity), false)
end

function buttonMainSafe(wd)
	if not customDataInited then return end
	text_typer.stopSounds(cfg.TextData)
...
function populateMissionList()
	...
	elseif noMissions then
		...
		if customData.noMissionsSpeech then
			text_typer.init(cfg.TextData, customData.noMissionsSpeech.text or cfg.Speech.noMissionsSpeech.text, customData.chatterSound or cfg.TextData.sound)
			...
			text_typer.init(cfg.TextData, cfg.Speech.noMissionsSpeech.text, customData.chatterSound or cfg.TextData.sound)
	...
	if selected then
		...
		if listData then
			...
			if dat then
				...
				text_typer.init(cfg.TextData, text, customData.chatterSound or cfg.TextData.sound)
...
function populateCrewList()
	if cfg.Data.crew and type(cfg.Data.crew) == "table" then
		...
		if #cfg.Data.crew > 0 then
			...
			text_typer.init(cfg.TextData, cfg.TextData.strings.noCrew, customData.chatterSound or cfg.TextData.sound)
			if customData.noCrewSpeech then
				text_typer.init(cfg.TextData, customData.noCrewSpeech.text or cfg.Speech.noCrewSpeech.text, customData.chatterSound or cfg.TextData.sound)
				...
				text_typer.init(cfg.TextData, cfg.Speech.noCrewSpeech.text, customData.chatterSound or cfg.TextData.sound)
		...
		if listData then
			...
			elseif listData[1] == "customAI" then
				if root.itemConfig("apexancientaichip") then
					resetGUI()
					text_typer.init(cfg.TextData, cfg.TextData.strings.customAI, customData.chatterSound or cfg.TextData.sound)
					...
					resetGUI()
					text_typer.init(cfg.TextData, cfg.TextData.strings.noCustomAIMod, customData.chatterSound or cfg.TextData.sound)
...
function exButtonMissions()
	GUI.crewInView = false
end

function exSwitchMissionMain()
...
function exSwitchMissionSecondary()
	if GUI.crewInView then
		resetGUI()
...
function exDismissConfirm()
	if GUI.crewViewing == "inactive" then
		world.sendEntityMessage(player.id(), 'dismissStoredCompanion', cfg.Data.selectedCrewID)
	end
end

function exCrewSelectedSafe(crew)
	widget.setText("path", "root/sail/ui/crew/"..GUI.crewViewing.."/"..crew.name)
end

function exPopulateCrewList()
	if GUI.crewViewing == "inactive" then
		text_typer.init(cfg.TextData, cfg.TextData.strings.noStoredCrew, customData.chatterSound or cfg.TextData.sound)
		if customData.noStoredCrewSpeech then
			text_typer.init(cfg.TextData, customData.noStoredCrewSpeech.text or cfg.Speech.noStoredCrewSpeech.text, customData.chatterSound or cfg.TextData.sound)
			...
			text_typer.init(cfg.TextData, cfg.Speech.noStoredCrewSpeech.text, customData.chatterSound or cfg.TextData.sound)
