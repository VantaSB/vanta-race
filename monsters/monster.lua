...
-- This is called in update() using pcall to catch errors
function update(dt)
  ...
  if self.behaviorTick >= self.behaviorTickRate then
    ...
    self.damageParts = {}
    --BData:clearControls() BData callbacks are no longer used, kept here for comparison/historical reference
    clearAnimation()
    ...
    if self.behavior then
      ...
      --self.behavior:run(self.behaviorState, dt * self.behaviorTickRate) <-- Old AF

      self.behavior:run(dt * self.behaviorTickRate)
    end

    --BData:update() <-- HELLA Old
    BGroup:updateGroups()
...
function interact(args)
  ...
  --BData:setEntity("interactionSource", args.sourceId) *Stares in BAH GAWD*
